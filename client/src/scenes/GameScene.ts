...
export class GameScene extends Phaser.Scene {
  ...
  async create() {
    ...
    // Force initial update to default location (Hanoi) so map isn't blank waiting for GPS
    // Only if we don't have a last known position
  ...
  private async fetchCharacterData() {
    try {
      // For dev, if ID is fake/local, we simulate data
      // const data = await RestClient.get<Character>(`/player/${this.characterId}`);
      
      // Mock data if API fails or running without backend DB for UI testing
  ...
  private startGPS() {
    ...
    this.gpsTracker?.startTracking((result) => {
      // Log for debug
      // console.log('GPS Update:', result.currentPosition);
